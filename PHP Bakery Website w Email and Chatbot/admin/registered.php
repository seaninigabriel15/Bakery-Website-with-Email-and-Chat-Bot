...
if (isset($_GET['delete_cid'])) {
  $delete_cid = (int) $_GET['delete_cid'];
  $stmt = "DELETE FROM customer WHERE customer_id =".$delete_cid;
  if ($conn->query($stmt) === TRUE) {
    header('location:registered.php?deleted');
} else {
...
<html>
  <head>
    <title>Kezha Bake ADMIN | Customers </title>
...
<div class="nav">
  <ul>
		  ...
		  <li><a href="cakes.php">Cakes</a></li>
           <li class="active"><a href="registered.php">Registered</a></li>
        </ul>
</div>
<div class="row">
<form method="post">
...
<div class="form" style="margin: 3em;">
    <div class="title">
    <label style="font-size: 20px;">Registered</label>
  </div>  
...
<div align="right" class="sear col s6" style="margin-top: 3em;">
  <label>Search:</label>
   ...
   if(isset($_POST['search'])){
    $sub = $_POST['txtsearch'];
     $sql = "SELECT * FROM customer WHERE customer_name like '%$sub%' OR email_address like '%$sub%'";
...
?>
      <input type="text" class="search" placeholder="Search(ID | Email)" name="txtsearch" style="height: 3em;"></input>
    </div>  
...
?>
  ...
  <tbody>
          ...
          <td><?php echo $row["customer_name"]?></td>
           ...
           <td>  <center>
           <a href='edit-customer.php?cid=<?php echo $row['customer_id'];?>' class='pen' style="padding: 1px">&#9998</a>&nbsp&nbsp
           <input type="text" name="cust_id" value=<?php echo $row["customer_id"] ?> style="display:none;">
           </td>
		   <td><center><a href='?delete_cid=<?php echo $row['customer_id'];?>'
